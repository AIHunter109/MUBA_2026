...
export default function GuardiansScreen() {
  ...
  return <AppPage title="Guardians" subtitle="Trusted wallets can review protected payments before money moves."><View className="gap-3">
    <View className="flex-row gap-2"><ModeButton label="Add guardian" selected={viewMode === 'create'} onPress={() => setViewMode('create')} /><ModeButton label="Review guardians" selected={viewMode === 'review'} onPress={() => setViewMode('review')} /></View>
...
function ModeButton({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) { return <Pressable accessibilityRole="button" accessibilityState={{ selected }} onPress={onPress} className={`flex-1 rounded-xl border px-3 py-3 ${selected ? 'border-blue-400 bg-blue-400/10' : 'border-slate-700 bg-slate-900/70'}`}><Text className={`text-center font-semibold ${selected ? 'text-blue-300' : 'text-slate-300'}`}>{label}</Text></Pressable>; }
