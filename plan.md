...
Auth shell now runs in Expo Go: `AuthProvider` context, `Stack.Protected` route guards, an `(app)` authenticated group, a working sign-in screen, and persisted sessions (SecureStore native, sessionStorage web). Until Enoki credentials exist, `lib/auth/demo-auth.ts` signs in by generating a local Ed25519 keypair (`expo-crypto`) and deriving a real Sui testnet address. `@mysten/dapp-kit` / `registerEnokiWallets` were removed from the native path because they are browser-only; the real Enoki client will slot in behind `AuthClient` in `lib/auth/auth-context.tsx` without changing screens.
